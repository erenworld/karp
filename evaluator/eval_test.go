...
func TestEvalComplexIntegerExpressions(t *testing.T) {
	tests := []struct {
		input    string
		expected int64
	}{
...
func TestFunctionObject(t *testing.T) {
	...
	if fn.Body.String() != expectedBody {
		t.Fatalf("body is nto %q. got=%q", expectedBody, fn.Body.String())
	}
}
...
func TestBuiltinFunctions(t *testing.T) {
	...
	for _, tt := range tests {
		...
		case string:
			errObj, ok := eval.(*object.Error)
			if !ok {
				t.Errorf("object is not Error. go=%T (%+v)", eval, eval)
				continue
			}
...
func TestIndexHashExpressions(t *testing.T) {
	tests := []struct{
		input 	 string
		expected interface{}
	}{
